...
export default defineComponent({
  ...
  setup(props, { attrs, emit, slots, expose }) {
    ...
    const renderPreview = (url: string, field: string) => {
      ...
      return <div class="flex items-center">{imagePreview.includes(field) ? <ElImage src={url} fit="cover" class="w-[100%]" lazy preview-src-list={[url]} preview-teleported /> : null}</div>
    }
    ...
    return () => {
      ...
      return (
        <div v-loading={unref(getProps).loading}>
          ...
          {unref(getProps).pagination ? (
            <ElPagination v-model:pageSize={pageSizeRef.value} v-model:currentPage={currentPageRef.value} class="my-10px flex justify-end" {...unref(pagination)}></ElPagination>
          ) : undefined}
        </div>
      )
    }
