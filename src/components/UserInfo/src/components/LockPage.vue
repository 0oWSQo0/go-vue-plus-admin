    ...
    <transition name="fade-slide">
      <div :class="`${prefixCls}-entry`" v-show="!showDate">
        <div :class="`${prefixCls}-entry-content`">
          ...
          <span :class="`text-14px ${prefixCls}-entry__err-msg enter-x`" v-if="errMsg">锁屏密码错误</span>
          <div :class="`${prefixCls}-entry__footer enter-x`">
            <ElButton type="primary" size="small" class="mt-2 mr-2 enter-x" link :disabled="loading" @click="handleShowForm(true)"> 返回 </ElButton>
            <ElButton type="primary" size="small" class="mt-2 mr-2 enter-x" link :disabled="loading" @click="goLogin"> 返回登录 </ElButton>
            <ElButton type="primary" class="mt-2" size="small" link @click="unLock()" :disabled="loading"> 进入系统 </ElButton>
          </div>
        </div>
      </div>
    </transition>
